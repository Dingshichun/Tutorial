...
auto mid = v2.begin() + v2.size() / 2; // 中间的迭代器
...
cout << count << endl;
```
**建议：尽量使用标准库类型而非数组，使用指针和数组很容易出错。**  
一部分原因是概念上的问题：指针常用于底层操作，因此容易引发一些与繁琐细节有关的错误。  
...
而且由于没有函数体，可以只写形参类型，不需要写形参名称，比如： `int func(int,int);`  
